...
runonce.run("/usr/lib/policykit-1-gnome/polkit-gnome-authentication-agent-1")
...
-- {{{ Tags
tags = {
  names = { "1:SHELL", "2:WORK ", "3:WORK", "4:MUSIC ", "5:BROWSER", 6,7,8,9 }
}
...
kbdcfg.updatetext = function()
  kbdcfg.widget:set_markup(markup(blue,"Kbd") .." " .. kbdcfg.layout[kbdcfg.current][1] .. " ")
end
kbdcfg.updatetext()
...
-- kbdcfg.widget:set_bgimage(beautiful.widget_bg)
keyboardwidget = wibox.widget.background()
keyboardwidget:set_widget(kbdcfg.widget)
keyboardwidget:set_bgimage(beautiful.widget_bg)
...
for s = 1, screen.count() do
  ...
  mytaglistwidget[s] = wibox.widget.background()
  mytaglistwidget[s]:set_widget(mytaglist[s])
  mytaglistwidget[s]:set_bgimage(beautiful.widget_bg)

  left_layout:add(mytaglistwidget[s])
  left_layout:add(spr_small)
  left_layout:add(mylayoutbox[s])
  -- left_layout.set_bg("#bada55")
  ...
  bottom_right_layout:add(keyboardwidget);
  ...
  lain.widgets.borderbox(mybottomwibox[s], s, { position = "top", color = "#242424" } )
end
-- }}}
...
globalkeys = awful.util.table.join(
  ...
  awful.key({ altkey, "Control" }, "-", function () lain.util.useless_gaps_resize(-1) end),
  awful.key({ modkey, "Shift" }, "o" , function () kbdcfg.switch() end),
  ...
  -- i'm using modified dmenu_run: https://github.com/syl20bnr/animated-dmenu
  awful.key({ modkey }, "d", function () os.execute("dmenu_run -i -nb '#000000' -nf '#ffffff' -sb '#000000' -sf '#1793d0' -p 'Exec: ' -fn '-xos4-terminus-*-*-*-*-40-*-*-*-*-*-iso10646-1' -z -a -h 32") end),
  ...
  awful.key({"Shift", }, "XF86AudioRaiseVolume", function()
      awful.util.spawn("amixer set Speaker 5%+", false)
      myvolumebar.notify()
    end),
  ...
  awful.key({ }, "XF86AudioRaiseVolume", function()
      ...
      awful.util.spawn("amixer set Headphone 5%+", false)
      myvolumebar.notify()
    end),
  ...
  awful.key({ }, "XF86AudioMute", function()
      ...
      awful.util.spawn("amixer set Master toggle & amixer set Headphone toggle", false)
      awful.util.spawn("amixer set Headphone toggle", false)
      awful.util.spawn("amixer set Speaker 0%", false)
...
for i = 1, 9 do
  globalkeys = awful.util.table.join(globalkeys,
    ...
    awful.key({ modkey, }, "F"..i,
      function ()
        if client and client.focus then
          local tag = awful.tag.gettags(client.focus.screen)[i]
          if client and client.focus and tag then
            awful.client.movetotag(tag)
          end
        end
      end)
